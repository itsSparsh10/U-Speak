...
export function AddEmployee({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const [step, setStep] = useState<'basic' | 'details' | 'custom' | 'license' | 'review'>('basic');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const employeeData = {
        ...
        ...(user.corporateAccountId && { corporateAccountId: user.corporateAccountId })
      };

      console.log('Creating employee with data:', employeeData);
      console.log('User corporate account ID:', user.corporateAccountId);
  ...
  const getStepStatus = (stepName: string) => {
    ...
    if (['basic', 'details', 'custom', 'license', 'review'].indexOf(stepName) < ['basic', 'details', 'custom', 'license', 'review'].indexOf(step)) {
      return 'completed';
    }
    return 'pending';
  };
        ...
        <Card className="bg-white shadow-xl">
          ...
          <CardContent className="p-6">
            ...
            <div className="flex items-center justify-center space-x-4 mb-8">
              {['basic', 'details', 'custom', 'license', 'review'].map((stepName, index) => (
              ...
              <div className="space-y-6">
                <div className="bg-blue-50 rounded-lg p-4">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <h4 className="font-medium text-blue-900">Custom Attributes</h4>
                      <p className="text-sm text-blue-700 mt-1">
                        Define up to three custom attributes to organize and filter employees in reports.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    <Label htmlFor="customAttribute1" className="text-sm font-medium text-gray-700">
                      {customAttributeLabels.attribute1}
                    </Label>
                    <Input
                      id="customAttribute1"
                      value={formData.customAttribute1}
                      onChange={(e) => handleInputChange('customAttribute1', e.target.value)}
                      className="mt-1"
                      placeholder="e.g., North Division"
                    />
                  </div>
                  <div>
                    <Label htmlFor="customAttribute2" className="text-sm font-medium text-gray-700">
                      {customAttributeLabels.attribute2}
                    </Label>
                    <Input
                      id="customAttribute2"
                      value={formData.customAttribute2}
                      onChange={(e) => handleInputChange('customAttribute2', e.target.value)}
                      className="mt-1"
                      placeholder="e.g., Field Sales"
                    />
                  </div>
                  <div>
                    <Label htmlFor="customAttribute3" className="text-sm font-medium text-gray-700">
                      {customAttributeLabels.attribute3}
                    </Label>
                    <Input
                      id="customAttribute3"
                      value={formData.customAttribute3}
                      onChange={(e) => handleInputChange('customAttribute3', e.target.value)}
                      className="mt-1"
            ...
            {step === 'review' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <h4 className="font-medium text-gray-900">Location & Attributes</h4>
                    <div className="space-y-2 text-sm">
                      <div><span className="font-medium">Location:</span> {formData.city}, {formData.state}, {formData.country}</div>
                      <div><span className="font-medium">Division:</span> {formData.customAttribute1 || 'Not set'}</div>
                      <div><span className="font-medium">Function:</span> {formData.customAttribute2 || 'Not set'}</div>
                      <div><span className="font-medium">Role Level:</span> {formData.customAttribute3 || 'Not set'}</div>
                    </div>
                  </div>
                </div>
