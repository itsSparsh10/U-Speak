...
export default function EmployeeDashboardPage() {
  ...
  const [firstNameInput, setFirstNameInput] = useState<string>(currentUser?.firstName || '');
  const [lastNameInput, setLastNameInput] = useState<string>(currentUser?.lastName || '');
  const [emailInput, setEmailInput] = useState<string>(currentUser?.email || '');
  const [phoneInput, setPhoneInput] = useState<string>('');
  const [departmentInput, setDepartmentInput] = useState<string>(currentUser?.department || '');
  const [jobTitleInput, setJobTitleInput] = useState<string>(currentUser?.jobTitle || '');

  // Keep form inputs in sync when currentUser changes
  useEffect(() => {
    if (currentUser) {
      ...
      setJobTitleInput(currentUser.jobTitle || '');
    }
  }, [currentUser]);
